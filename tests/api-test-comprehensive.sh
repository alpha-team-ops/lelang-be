...
# TEST 11: Verify token after logout (access token still valid)
...
echo -e "${BLUE}TEST 15: Login as moderator and verify role permissions${NC}"
MODERATOR_LOGIN=$(curl -s -X POST "$API_URL/auth/login" \
    -H "Content-Type: application/json" \
    -d '{"email":"moderator@deraly.id","password":"SecurePassword123!"}')

MODERATOR_ROLE=$(echo $MODERATOR_LOGIN | jq -r '.data.role // empty')
MODERATOR_PERMISSIONS=$(echo $MODERATOR_LOGIN | jq -r '.data.permissions | length // empty')

if [ "$MODERATOR_ROLE" = "MODERATOR" ] && [ "$MODERATOR_PERMISSIONS" -gt 0 ]; then
    echo -e "  ${GREEN}✓ PASSED (role: $MODERATOR_ROLE, permissions: $MODERATOR_PERMISSIONS)${NC}"
    ((TESTS_PASSED++))
else
    echo -e "  ${RED}✗ FAILED${NC}"
    ((TESTS_FAILED++))
...
# TEST 16: Verify admin has more permissions than moderator
echo -e "${BLUE}TEST 16: Verify ADMIN has more permissions than MODERATOR${NC}"
ADMIN_PERMS=$(echo $LOGIN_RESPONSE | jq -r '.data.permissions | length // empty')
MOD_PERMS=$(echo $MODERATOR_LOGIN | jq -r '.data.permissions | length // empty')

if [ "$ADMIN_PERMS" -ge "$MOD_PERMS" ]; then
    echo -e "  ${GREEN}✓ PASSED (admin: $ADMIN_PERMS, moderator: $MOD_PERMS)${NC}"
    ((TESTS_PASSED++))
else
    echo -e "  ${RED}✗ FAILED${NC}"
    ((TESTS_FAILED++))
