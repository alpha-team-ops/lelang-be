...
echo -e "${YELLOW}========== Authentication API v1 Comprehensive Tests ==========${NC}\n"

# TEST 1: Login with valid credentials
test_endpoint 1 "POST" "/auth/login" \
    '{"email":"alpha.dev@deraly.id","password":"AnotherPassword789!"}' \
    "" \
    "true" \
...
LOGIN_RESPONSE=$(curl -s -X POST "$API_URL/auth/login" \
    -H "Content-Type: application/json" \
    -d '{"email":"alpha.dev@deraly.id","password":"AnotherPassword789!"}')

ACCESS_TOKEN=$(echo $LOGIN_RESPONSE | jq -r '.data.accessToken')
REFRESH_TOKEN=$(echo $LOGIN_RESPONSE | jq -r '.data.refreshToken')

echo -e "${YELLOW}Obtained Tokens:${NC}"
...
test_endpoint 3 "POST" "/auth/login" \
    '{"email":"alpha.dev@deraly.id","password":"WrongPassword123"}' \
    "" \
    "false" \
...
# TEST 5: Verify with invalid token (protected, should fail)
...
test_endpoint 6 "POST" "/auth/change-password" \
    '{"currentPassword":"AnotherPassword789!","newPassword":"NewPassword555!"}' \
    "Authorization: Bearer $ACCESS_TOKEN" \
    "true" \
    "Change password"
...
# TEST 9: Register with duplicate email (should fail)
...
# TEST 11: Verify token after logout (access token still valid - not blacklisted)
test_endpoint 11 "GET" "/auth/verify" \
    "" \
    "Authorization: Bearer $NEW_ACCESS_TOKEN" \
    "true" \
    "Verify token after logout (access token still valid until expiry)"
...
# TEST 13: Register with weak password (should fail)
...
# TEST 14: Register with non-existent organization (should fail)
...
# TEST 15: Moderator login and verify token
echo -e "${BLUE}TEST 15: Moderator login and verify permissions${NC}"
MOD_LOGIN=$(curl -s -X POST "$API_URL/auth/login" \
    -H "Content-Type: application/json" \
    -d '{"email":"moderator@deraly.id","password":"SecurePassword123!"}')

MOD_TOKEN=$(echo $MOD_LOGIN | jq -r '.data.accessToken')
MOD_VERIFY=$(curl -s -X GET "$API_URL/auth/verify" \
    -H "Authorization: Bearer $MOD_TOKEN")

MOD_ROLE=$(echo $MOD_VERIFY | jq -r '.data.user.role')
MOD_PERMS=$(echo $MOD_VERIFY | jq -r '.data.user.permissions | length')

if [ "$MOD_ROLE" = "MODERATOR" ]; then
    echo -e "  ${GREEN}✓ PASSED (role: $MOD_ROLE, permissions: $MOD_PERMS)${NC}"
    ((TESTS_PASSED++))
else
    echo -e "  ${RED}✗ FAILED${NC}"
    ((TESTS_FAILED++))
...
# TEST 16: Verify admin has correct role and permissions
echo -e "${BLUE}TEST 16: Admin login and verify permissions${NC}"
ADMIN_VERIFY=$(curl -s -X GET "$API_URL/auth/verify" \
    -H "Authorization: Bearer $ACCESS_TOKEN")

ADMIN_ROLE=$(echo $ADMIN_VERIFY | jq -r '.data.user.role')
ADMIN_PERMS=$(echo $ADMIN_VERIFY | jq -r '.data.user.permissions | length')

if [ "$ADMIN_ROLE" = "ADMIN" ] && [ "$ADMIN_PERMS" -gt 0 ]; then
    echo -e "  ${GREEN}✓ PASSED (role: $ADMIN_ROLE, permissions: $ADMIN_PERMS)${NC}"
    ((TESTS_PASSED++))
else
    echo -e "  ${RED}✗ FAILED${NC}"
    ((TESTS_FAILED++))
...
if [ $TESTS_FAILED -eq 0 ]; then
    ...
    echo -e "${GREEN}API is ready for production.${NC}"
    exit 0
else
